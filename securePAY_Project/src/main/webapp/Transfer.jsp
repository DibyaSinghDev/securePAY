...
.transfer-panel input#amount{padding-left:38px}
.transfer-panel input:focus{border-color:var(--gold);outline:2px solid var(--gold-soft);outline-offset:1px}
#errorMsg{
...
.transfer-btn:hover{background:var(--ink-2)}
.transfer-btn:focus-visible{outline:2px solid var(--gold);outline-offset:2px}
...
@media(max-width:900px){
  ...
  .container{width:94%}
  .balance-card,.transfer-panel{padding:26px 22px}
...
<div class="panel-title">Enter transfer details</div>
<% if(error!=null){ %>
...
<input type="text" id="receiverAccount" name="receiverAccount" required placeholder="Enter account number">
</div>
...
amount.addEventListener("input",function(){
 ...
 if(value>balance){
   ...
   this.style.borderColor="#B23A2E";
 }else{
   error.innerHTML="";
   this.style.borderColor="#1E7B4D";
 }
});
